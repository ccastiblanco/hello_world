...
## operaciones basicas: aritmpeticas, input/output, soporte a programacion básica. Es el que viene instalado predefinido
...
```
c es para crear el vector, debe ser del mismo tipo de datos
...
## typeof dice cual es el tipo del objeto
typeof(x)
x <-c(TRUE, FALSE) #logico
typeof(x)
x <-c("a", "b", "c") #caracter
typeof(x)
x <-c(10:20) # integer/entero del 10 al 20, slice, va de uno en uno porque es por defecto
typeof(x)
...
vector1 <- c("cali", "medellin","barranquilla", "bogotá")
...
for (number in array)
  {
    if(number%%2==0) # %% genera el residuo de una division
    {
      print(number)  
    }
  }
...
```
R1/ 
Se obtiene el valor de la posicion 3, en este caso es la "h"
R2/
Se pide que extraiga un subconjunto de datos, en este caso de las posiciones 1,3 y 5, que son el "k","h" u "c"
R3/
Se obtiene un slice o rango de datos, desde la posicion 2 hasta la posicion 6 de la matriz: "j" "h" "a" "c" "m"
...
x[5 , 3] <- 22 # Cambio el valor en esa posición

x[ ,2] <- 50 # toda la columna 2 queda con valor 50
...
#flights  #este no existe, para que exista en mi propio entorno, debo asignarle el anterior
...
srt Compactly Display the Structure of an Arbitrary R Object
...
diabetesFactor1 <- factor(diabetes2) 
```
...
```{R factorExample3}
x2 <- factor(c("yes", "yes", "no", "yes", "no"), levels = c("yes", "no"))
```
_Ejercicio 10_
...
PatientStatus
```
OJO! Para elmiminar de la memoria: rm(variable)
...
```{R ejercicioFactor2}
# 1 linea de código
xx <- factor(c(1, 2, 3, 3, 5, 3, 2, 4, NA) )
...
R/
Los valores atipicos no tienen niveles en las variables categóricas. es decir, un valor NA queda sin nivel
...
```{R ejercicioFactor3}
# 2 lineas de código
x3 <- c(11, 22, 47, 47, 11, 47, 11)
x3factor <- factor(x3, levels=c(11,22,47) , ordered=TRUE )
x3factor                   
...
mylist[[3]][1,2] #retorna un solo valor de la matriz
mylist[["ages"]]
```
...
la nomenclatura "```" es para decir en markdown que es un codigo en R, pero no se ejecuta. Para que se ejecute, debo crear la nomenclatura "```{R indice}" , donde indice es un nombre especifico y unico
